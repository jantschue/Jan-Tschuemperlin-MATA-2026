"""
Hyperparameter-Tuning für das MLP-v8-Modell mittels Optuna.
Tuning läuft ausschliesslich auf 050_Brunnen_Mositunnel_R1 (v8-Daten mit den
26 kantonsspezifischen Schulferienspalten).
Die besten Hyperparameter werden in results/model_results/mlp_v8/best_params_v8.json gespeichert.
...
RESULTS_DIR = Path("results/model_results/mlp_v8")
...
def main():
    ...
    # Optuna-Studie samt SQLite-Datenbank im Modell-Ordner ablegen (neben best_params_v8.json),
    # damit der Projekt-Root sauber bleibt. Ordner vor der Study-Erstellung sicherstellen.
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    storage_url = f"sqlite:///{(RESULTS_DIR / 'optuna_mlp_v8.db').as_posix()}"
    ...
    sep = "-" * 48
    print(f"\n{sep}")
    print("Beste Hyperparameter – manuell in models/mlp_v8.py eintragen:\n")
    ...
    best_params_path = RESULTS_DIR / "best_params_v8.json"
