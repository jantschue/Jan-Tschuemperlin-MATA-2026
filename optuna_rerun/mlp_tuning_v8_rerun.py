"""
Rerun des Optuna-Hyperparameter-Tunings für das MLP-v8-Modell (GPU-Variante).
Identisch zu models/mlp_tuning_v8.py, mit folgenden Anpassungen:
  * Ausgabepfade und Study-Name isoliert nach optuna_rerun/ (Suffix _gpu),
    damit weder die produktiven Ergebnisse noch die abgebrochene CPU-Vorab-DB
    (optuna_rerun.db) überschrieben werden.
  * Harte CUDA-Pflicht: bricht sofort ab, falls kein GPU-Backend verfügbar ist.
  * Ampere-Optimierungen (TF32 + cuDNN-Benchmark) für RTX 3060 Ti.
  * Datensatz wird komplett ins VRAM preloaded (bereits Verhalten des Originals).

Aufruf aus dem Repo-Root:  python optuna_rerun/mlp_tuning_v8_rerun.py

Tuning-Logik, N_TRIALS, TUNING_EPOCHS, Suchräume, Sampler und Pruner sind
gegenüber models/mlp_tuning_v8.py unverändert.
"""
...
# GPU-Pflicht: dieser Rerun ist explizit für den RTX-3060-Ti-Lauf zuhause gedacht.
# Wenn CUDA nicht verfügbar ist, sofort abbrechen (statt still auf CPU zurückzufallen),
# damit ein fehlender/kaputter CUDA-PyTorch-Build sofort auffällt.
if not torch.cuda.is_available():
    raise SystemExit(
        "CUDA nicht verfügbar. Dieser Rerun muss auf der GPU laufen.\n"
        "Bitte einen CUDA-fähigen PyTorch-Build installieren "
        "(z.B. `pip install torch --index-url https://download.pytorch.org/whl/cu121`)."
    )

device = torch.device("cuda")

# Ampere-Optimierungen (RTX 3060 Ti = Ampere): TF32-Matmuls und cuDNN-Autotuner
# beschleunigen die vielen kleinen Trainingsläufe spürbar, ohne die Ergebnisse
...
def main():
    ...
    print(f"Optuna Hyperparameter-Tuning (RERUN – GPU) – Device: {device}")
    ...
    # Alle vier Tensoren leben ab hier komplett im VRAM – die Trainingsschleife
    # macht nur noch Slicing, kein CPU->GPU-Transfer mehr.
    ...
    preload_mb = sum(t.element_size() * t.numel() for t in (X_train_g, y_train_g, X_val_g, y_val_g)) / 1024**2
    torch.cuda.synchronize()
    alloc_mb   = torch.cuda.memory_allocated() / 1024**2
    print(f"GPU-Preload fertig: Datensatz-Tensoren {preload_mb:.1f} MB  |  aktuell allokiert: {alloc_mb:.1f} MB\n")

    optuna.logging.set_verbosity(optuna.logging.WARNING)

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    # Eigene DB + Study-Name für den GPU-Lauf, damit die abgebrochene CPU-Vorab-DB
    # (optuna_rerun.db) unangetastet als Isolationsnachweis liegen bleibt.
    study = optuna.create_study(
        study_name="mlp_v8_tuning_rerun_gpu",
        storage="sqlite:///optuna_rerun/optuna_rerun_gpu.db",
        load_if_exists=True,
        direction="minimize",
    ...
    sep = "-" * 48
    print(f"\n{sep}")
    print("Beste Hyperparameter (Rerun):\n")
    ...
    best_params_path = RESULTS_DIR / "best_params_v8_rerun_gpu.json"
    with open(best_params_path, "w") as f:
        json.dump(best_params, f, indent=4)
    print(f"\nGespeichert: {best_params_path}")

    dauer_path = RESULTS_DIR / "tuning_dauer_gpu.txt"
    with open(dauer_path, "w") as f:
        f.write(f"tuning_seconds: {duration:.3f}\n")
        f.write(f"tuning_hms:     {duration_hms}\n")
        f.write(f"n_trials_completed: {len(completed)}\n")
        f.write(f"n_trials_pruned:    {len(pruned)}\n")
