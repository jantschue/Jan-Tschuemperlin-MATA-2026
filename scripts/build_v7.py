...
v6-Dateien (_v6.csv), ersetzt die binäre is_holiday-Spalte durch
...
DATA_DIR     = BASE_DIR / "data" / "v6"
HOLIDAYS_CSV = BASE_DIR / "data" / "holidays" / "swiss_holidays_2015_2025.csv"
OUT_DIR      = BASE_DIR / "data" / "v7"

DATASETS = [
    "050_Brunnen_Mositunnel_R1_v6.csv",
    "050_Brunnen_Mositunnel_R2_v6.csv",
    "171_Sattel_R1_v6.csv",
    "171_Sattel_R2_v6.csv",
    "216_Wangen_SZ_R1_v6.csv",
    "216_Wangen_SZ_R2_v6.csv",
    "299_Wollerau_Blatttunnel_R1_v6.csv",
    "299_Wollerau_Blatttunnel_R2_v6.csv",
    "720_Schwyz_R1_v6.csv",
    "720_Schwyz_R2_v6.csv",
]
...
def sanity_check(filename: str, df: pd.DataFrame):
    ...
    print(f"\n-- Sanity-Check: {filename.replace('_v6', '_v7')} --")
...
def main():
    ...
    for filename in DATASETS:
        ...
        out_name = filename.replace("_v6", "_v7")
        out_path = OUT_DIR / out_name
        df.to_csv(out_path, index=False)
        print(f"Geschrieben: {out_path}  ({len(df)} Zeilen, {len(df.columns)} Spalten)")

    df_check = pd.read_csv(OUT_DIR / DATASETS[0].replace("_v6", "_v7"))
