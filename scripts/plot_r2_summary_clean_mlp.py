"""
Dieses Skript erzeugt ein aufgeraeumtes, thesenreifes R2-Balkendiagramm des
MLP-Hauptmodells (v8) fuer Kapitel 4.1. Es benoetigt kein PyTorch, sondern liest
ausschliesslich die bereits berechnete Metrik-Datei
results/model_results/mlp_v8/metrics/all_metrics.csv und stellt pro Datenreihe
das Bestimmtheitsmass R2 dar. Die x-Achse zeigt lesbare Stationsnamen statt der
Dateinamen, die Balkenwerte werden in Schweizer Schreibweise (Komma) beschriftet.

Da das MLP deutlich hoehere R2-Werte erreicht als die lineare Baseline, reicht
die y-Achse hier bis 1,0 (bei der linearen Regression bis 0,8).

Ergebnis:
    results/model_results/mlp_v8/summary/r2_summary_clean.png
"""
...
# ── Konfiguration ────────────────────────────────────────────────────────────
METRICS_CSV = Path("results/model_results/mlp_v8/metrics/all_metrics.csv")
OUT_PATH    = Path("results/model_results/mlp_v8/summary/r2_summary_clean.png")

BAR_COLOR      = "#4C72B0"   # neutrale Einheitsfarbe (kein Gruen)
Y_MIN, Y_MAX   = 0.0, 1.0    # y-Achse startet bei 0 (MLP erreicht bis ~0,96)
