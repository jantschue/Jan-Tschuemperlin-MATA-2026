...
| `results/data_visualizations/` | Diagnostik der Datenpipeline: Korrelationsmatrizen (`correlation_analysis/`), Tagesverlaufs-Plots (`hourly_plots/`) und Lücken-Übersicht (`gaps.txt`) |
| `webapp/` | Interaktive React-Webapp (Vite + Tailwind + Recharts); live unter [tschue.ch](https://tschue.ch) |
| `anhang/` | Formatierte Quellcode-Fassungen der beiden v8-Modellskripte (lineare Regression, MLP) für den Anhang der schriftlichen Arbeit; eigenständige Kopien, nicht Teil von `run_pipeline.py` |
| `optuna_rerun_20260731/` | Isolierter Optuna-Re-Run zur Dokumentation von Tuning-Dauer und Ressourcen-Auslastung (CPU/RAM/GPU) für Kapitel 4.6; die massgebliche Architektur bleibt `results/model_results/mlp_v8/best_params_v8.json` — Details in `HINWEIS.txt` |
| `mlp_v8_test_20260731/` | Geschlossener Test des v8-MLP mit den Hyperparametern aus dem Optuna-Re-Run (liest nur `data/v8/`, schreibt ausschliesslich in diesen Ordner); belegt, dass die Re-Run-Architektur praktisch gleichwertige Resultate liefert |
| `Theorie/` | Zeilengenaue Erklärungsdateien zu den Modellskripten (Prüfungsvorbereitung, nicht Teil des produktiven Codes) |
| `Tutorials/` | Lernmaterialien, die ich während der Einarbeitung in Python, NumPy, Pandas und PyTorch erstellt habe |
| `Abbildungen_MATA/` | Abbildungen für die schriftliche Arbeit samt erzeugendem Code (`Code/`) und den fertigen Grafiken (`Abbildungen/`); eigenständige Skripte, nicht Teil von `run_pipeline.py`. Enthält u. a. die Zählstellen-Karte (`Code/plot_zaehlstellen_karte.py`, benötigt `geopandas`/`contextily`, die Kantonsgrenze unter `data/geo/` sowie eine Internetverbindung für die Kartenkacheln von OpenStreetMap/CARTO), das Raster-Streudiagramm Prognose gegen Messwert je R1-Zählstelle (`Code/abbildung_prognose_vs_messwert_r1.py`, Abb. 18) und die R²-Veränderung durch den Corona-Ausschluss v5 vs. v6 (`Code/abbildung_r2_veraenderung_corona.py`, Abb. 20). `Code/collect_figures.py` sammelt alle erzeugten Abbildungen nummeriert (Abb_01 … Abb_23) im Übersichtsordner `Abbildungen_MATA/Alle_Abbildungen/` |
...
| `scripts/create_correlation_matrix_v8.py` | Aufgeräumte Pearson-Korrelationsmatrix je v8-Datenreihe für Kapitel 3.3.2: verdichtet die 26 Feiertags- und die 26 Schulferienspalten zu je einer Anteils-Sammelspalte und erstellt eine lesbare Heatmap über 17 Merkmale | `results/data_visualizations/correlation_analysis/{base}_v8_correlation_clean.csv` + `.png` |
| `scripts/plot_hourly_averages_clean.py` | Aufgeräumter durchschnittlicher Tagesverlauf je v8-Datenreihe (Temperatur, Sonnenscheindauer, Verkehrsvolumen über 24 h) mit aussagekräftigem Titel (ohne Stationscode/Versionskürzel) – thesenreife Variante von `plot_hourly_averages.py` | `results/data_visualizations/hourly_plots/{base}_v8_hourly_trend_clean.png` |
...
| `scripts/plot_trainingsverlauf_r2_schwyz_r1.py` | Trainingsverlauf-Abbildung für Schwyz R1 aus der von `mlp_v8.py` erzeugten `training_history_720_Schwyz_R1_v8.csv` im Stil der v7-Abbildungen: kombinierte R²- (blau, 0 bis 1) und Verlustkurve (rot, MSE) für Training/Validierung | `results/model_results/mlp_v8/plots/trainingsverlauf/trainingsverlauf_r2_720_Schwyz_R1_v8.png` |
...
| Unterordner | Inhalt |
|---|---|
| `metrics/` | MAE, RMSE und R² für alle Stationen als CSV; die v8-Modelle (`linear_regression_v8`, `mlp_v8`) speichern zusätzlich die Spalte `train_seconds` (gemessene Trainingszeit je Datenreihe, ausgewertet in Kapitel 4.6 Ressourcenverbrauch) |
...
| 14 | `plot_hourly_averages.py` | `v5/` → `results/data_visualizations/hourly_plots/` |
| 15 | `create_correlation_matrix_engineered.py` | `v5/` → `results/data_visualizations/correlation_analysis/` |

**Phase 2 – Analyse + Corona-Bereinigung + v7/v8 (7 Skripte):** `dataset_overview.py` und `covid_anomaly_analysis.py` erzeugen die Diagnostik unter `results/analysis/`; `build_v6.py` schreibt den Corona-bereinigten Datensatz nach `data/v6/`; `build_v7.py` erweitert v6 mit 26 kantonsspezifischen Feiertagsspalten aus der zentralen Feiertags-DB nach `data/v7/`; `build_v8.py` fügt 26 Schulferienspalten hinzu und speichert nach `data/v8/`. Direkt nach `build_v8.py` erstellt `create_correlation_matrix_v8.py` aus den fertigen v8-Daten die aufgeräumte Korrelationsmatrix (Feiertags- und Schulferienspalten je zu einer Anteils-Sammelspalte verdichtet) und speichert sie je Datenreihe als CSV und Heatmap nach `results/data_visualizations/correlation_analysis/`; `plot_hourly_averages_clean.py` erzeugt anschliessend die thesenreifen Tagesverlauf-Abbildungen (ohne technischen Titel/Versionskürzel) nach `results/data_visualizations/hourly_plots/`.

**Phase 3 – Modelltraining (8 Skripte + 7 Analysen/Abbildungen):** `linear_regression_v5.py` und `mlp_v5.py` trainieren je 10 Modelle auf v5, `linear_regression_v6.py` und `mlp_v6.py` trainieren mit identischen Hyperparametern dieselben Modelle auf v6. `linear_regression_v7.py` und `mlp_v7.py` trainieren Baseline und MLP auf den v7-Daten, `linear_regression_v8.py` und `mlp_v8.py` dieselben auf den v8-Daten. Beide neuen MLPs (v7, v8) nutzen eigene getunte Hyperparameter. Direkt danach liest `scripts/plot_trainingsverlauf.py` die von `mlp_v7.py` erzeugten `training_history/training_history_*.csv`-Dateien ein und erstellt je Station eine R²-/Verlust-Trainingsverlauf-Abbildung unter `plots/trainingsverlauf/`; anschliessend erzeugt `scripts/plot_trainingsverlauf_r2_schwyz_r1.py` aus der v8-Verlaufsdatei die kombinierte R²-/Verlust-Trainingsverlauf-Abbildung für Schwyz R1 im Stil der v7-Abbildungen unter `results/model_results/mlp_v8/plots/trainingsverlauf/`. Danach berechnet `scripts/permutation_importance.py` aus den trainierten v8-Gewichten die gruppierte Permutations-Wichtigkeit der Merkmalsgruppen und speichert CSV und Balkendiagramme unter `results/analysis/feature_importance/`. Zum Schluss erzeugen `scripts/plot_r2_summary_clean.py`/`scripts/plot_timeseries_clean.py` (lineare Regression) sowie `scripts/plot_r2_summary_clean_mlp.py`/`scripts/plot_timeseries_clean_mlp.py` (MLP) aus den jeweiligen v8-Metriken bzw. -Vorhersagen die aufgeräumten Abbildungen für Kapitel 4.1 (`r2_summary_clean.png`, `timeseries_clean_Schwyz_R1.png`) unter `results/model_results/linear_regression_v8/summary/` bzw. `results/model_results/mlp_v8/summary/`.
...
- **Bekannte Datenlücken:** Zwei Punkte in den Rohdaten sind bekannt und bewusst nicht korrigiert, weil alle veröffentlichten Ergebnisse darauf beruhen: der fehlende Endtermin der Schwyzer Sommerferien 2024 in der Schulferien-Tabelle und der Zeitzonenunterschied zwischen den MeteoSchweiz-Daten (UTC) und den ASTRA-Daten (Lokalzeit) beim Zusammenführen. Die Arbeit nennt beide Punkte in Kapitel 5.3.
...
- **Feiertags-Logik der Webapp:** `export_weights.py` und `webapp/src/utils/swissHolidays.js` verwenden denselben Butcher-Algorithmus für das Osterdatum und dieselbe Liste von 13 Schwyzer Feiertagen (mit Berchtoldstag, ohne Dreikönig und Josefstag). Die exportierten Feature-Vektoren und die in der Webapp angezeigten Feiertage sind damit untereinander konsistent, weichen aber vom Trainingsmerkmal `holiday_SZ` ab: Dieses stammt aus dem Modul `holidays` und umfasst 14 Feiertage pro Jahr, darunter Dreikönig (6.1.) und Josefstag (19.3.), jedoch nicht den Berchtoldstag. Die Live-Vorhersage (`LiveVorhersage.jsx`) prüft zudem nur acht Feiertage. Diese Abweichung ist bekannt und wurde vor der Abgabe nicht mehr angeglichen.
